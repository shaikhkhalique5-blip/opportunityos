...
async def build_seller_brain(req: DiscoveryRequest) -> SellerBrain:
    ...
    return await asyncio.to_thread(_openai_json, "seller_brain", SellerBrain, {
...
async def apollo_candidates(req: DiscoveryRequest, brain: SellerBrain) -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        try:
            ...
            return [{
                "company_name": x.get("name"), "website": x.get("website_url") or x.get("primary_domain"),
                "country": x.get("country"), "headcount": x.get("estimated_num_employees"),
                "industry": x.get("industry"), "provider": "apollo"
            } for x in rows if x.get("name") and (x.get("website_url") or x.get("primary_domain"))]
        except Exception:
            return []
...
async def public_candidates(req: DiscoveryRequest, brain: SellerBrain) -> list[dict]:
    if not settings.tavily_api_key:
        return []
    countries = ", ".join(req.icp.countries) or "target markets"
    industries = ", ".join(req.icp.industries or brain.target_industries[:5])
    signals = ", ".join(x.name for x in brain.trigger_signals[:8])
    async with httpx.AsyncClient(timeout=30) as client:
        docs = await tavily_search(client, f"companies {countries} {industries} {signals} automation transformation expansion hiring")
    candidates = []
    for d in docs:
        url = d.get("source_url", "")
        host = urlparse(url).netloc.replace("www.", "")
        if host:
            candidates.append({"company_name": d.get("source_name", host)[:120], "website": f"https://{host}", "provider": "public_web"})
    return candidates
...
async def research_candidate(candidate: dict, brain: SellerBrain) -> dict:
    ...
    return {"candidate": candidate, "research": docs[:20], "signal_taxonomy": [x.model_dump() for x in brain.trigger_signals]}
...
async def run_discovery(req: DiscoveryRequest) -> DiscoveryResponse:
    ...
    for row in apollo + public:
        ...
        if host and host not in seen:
            seen.add(host); merged.append(row)
    ...
    for item in evidence:
        account = await asyncio.to_thread(_openai_json, "ranked_account", RankedAccount, {
            "task": "Rank this account for the seller. Use only evidence provided. Infer buyer roles, not unsupported person identities. Set providers from candidate metadata.",
            "seller_brain": brain.model_dump(), "icp": req.icp.model_dump(), **item,
        })
    ...
    status = {
        ...
        "public_web": "connected" if settings.tavily_api_key else "limited_without_tavily",
